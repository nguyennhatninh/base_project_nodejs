...
export class ShopController {
  ...
  private async nearByShop(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const newResult: User[] = result.filter(
        (user) =>
          this.calcCrow(
            user.location.lat,
            user.location.log,
            parseFloat(lat.toString()),
            parseFloat(long.toString())
          ) <=
          parseInt(range.toString()) / 1000
      );
  ...
  private distance(lat1: number, lon1: number, lat2: number, lon2: number, unit = 'K') {
